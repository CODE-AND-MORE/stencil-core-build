...
function runReleaseTasks(opts, args) {
    ...
    if (!opts.isPublishRelease) {
        tasks.push({
            ...
            task: async () => {
                ...
                await execa_1.default('npm', ['version', '--no-git-tag-version', opts.version], { cwd: rootDir });
            },
        }, {
            title: `Generate ${opts.version} Changelog ${opts.vermoji}`,
            task: () => {
                return release_utils_1.updateChangeLog(opts);
            },
        });
    }
    if (opts.isPublishRelease) {
    ...
    if (opts.isPublishRelease) {
        tasks.push({
            ...
            task: () => {
                return release_utils_1.postGithubRelease(opts);
            },
        });
    }
